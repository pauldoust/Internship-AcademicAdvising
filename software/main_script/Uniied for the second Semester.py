...
#     need to be fix and replace FIEC01735 with target course
...
Exp1 = r"C:\Users\mpouldou\final-data\Experiment1\exp1_dataset.csv"
df = pd.read_csv(Exp1)
X = pd.DataFrame(df[df.columns.difference(['TargetGrade'])])
y = pd.DataFrame(df['TargetGrade'])
# print(y)
targetCourses = df['TargetCourse'].unique()
# print('targetCourses: ', targetCourses )
...
#     scores.append(score)
    ...
    test_set_label = y.iloc[test,:]
    y_pred = model.predict(test_set)
    y_true = test_set_label
    rmseScore = np.sqrt(metrics.mean_squared_error(y_true, y_pred))
    rmse.append(rmseScore)
#     for course in targetCourses:
    print('targetCourses: ', targetCourses)
    print('le.fit_transform(targetCourses): ', le.fit_transform(targetCourses))
    for course in le.fit_transform(targetCourses):
#         print('test: ', test_set[test_set['TargetCourse'] == course])
        testSet_target = test_set[test_set['TargetCourse'] == course]
        testSet_target_label = test_set_label[test_set['TargetCourse'] == course]
        y_pred_target = model.predict(testSet_target)   
        rmseScore = np.sqrt(metrics.mean_squared_error(testSet_target_label, y_pred_target))
        rmseCourse[str(course)].append(rmseScore)

#         rmseCourse[str(course) + "," + str(i)] = rmseScore
#         print(course)
    

print("Global: ", mean(rmse))
# print("Partial: ", mean(rmseCourse) )
for k,v in rmseCourse.items():
    print("Partial for Course", str(k), " = ", mean(v))
